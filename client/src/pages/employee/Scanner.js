...
const Scanner = () => {
  ...
  const requestCameraPermission = async () => {
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' } }); // Use rear camera by default
  ...
  const handleAddMoney = async (wallet_id, amountToAdd, otp) => {
    ...
    if (!isFourDigitNumber(otp)) {
      message.error('OTP should be a 4-digit number');
      return;
    }
  ...
  const handleDeviceChange = (deviceId) => {
    ...
    if (scannerRef.current) {
      ...
      scannerRef.current.startScan(deviceId);
    }
  };
          ...
          {permissionGranted && scannerVisible && (
            <>
              ...
              <Row justify="center" style={{ marginBottom: '10px' }}>
                <Col>
                  <Button onClick={() => handleDeviceChange(null)}>Switch Camera</Button>
                </Col>
              </Row>
